...
public_functions.addVehicle = function(description) {
  var vehicle_tuning = new Ammo.btVehicleTuning(), vehicle;
...
public_functions.removeObject = function(details) {
  ...
  var ptr = _objects[details.id].a != undefined
...
public_functions.simulate = function simulate(params) {
  if (world) {
    ...
    params.maxSubSteps = params.maxSubSteps ||
      Math.ceil(params.timeStep / fixedTimeStep); // If maxSubSteps is not defined, keep the simulation fully up to date
...
reportWorld = function() {
  var index, object, transform, origin, rotation, offset = 0, i = 0;
...
reportConstraints = function() {
  var index, constraint, offset_body, transform, origin, offset = 0, i = 0;

  if (SUPPORT_TRANSFERABLE) {
    if (
      constraintreport.length < 2 + _num_constraints * CONSTRAINTREPORT_ITEMSIZE
    ) {
      constraintreport = new Float32Array(
        2 + // message id & # objects in report
